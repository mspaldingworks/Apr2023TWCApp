 ...
 * this button in different states. See [TwcButtonDefaults.filledButtonColors].
 * @param contentPadding The spacing values to apply internally between the container and the
 * content. See [TwcButtonDefaults.buttonContentPadding].
...
fun TwcFilledButton(
    ...
    small: Boolean = false,
    colors: ButtonColors = TwcButtonDefaults.filledButtonColors(),
    contentPadding: PaddingValues = TwcButtonDefaults.buttonContentPadding(small = small),
    ...
    Button(
        ...
        modifier = if (small) {
            Modifier
                .heightIn(min = TwcButtonDefaults.SmallButtonHeight)
                .then(modifier)
        } else {
            modifier
        },
...
fun TwcFilledButton(
    ...
    TwcFilledButton(
        ...
        small = small,
        colors = colors,
        contentPadding = TwcButtonDefaults.buttonContentPadding(
            small = small,
 ...
 * @param colors [ButtonColors] that will be used to resolve the container and content color for
 * this button in different states. See [TwcButtonDefaults.outlinedButtonColors].
 * @param contentPadding The spacing values to apply internally between the container and the
 * content. See [TwcButtonDefaults.buttonContentPadding].
 * @param content The button content.
 */
@Composable
fun TwcOutlinedButton(
    onClick: () -> Unit,
    modifier: Modifier = Modifier,
    enabled: Boolean = true,
    small: Boolean = false,
    border: BorderStroke? = TwcButtonDefaults.outlinedButtonBorder(enabled = enabled),
    colors: ButtonColors = TwcButtonDefaults.outlinedButtonColors(),
    contentPadding: PaddingValues = TwcButtonDefaults.buttonContentPadding(small = small),
    content: @Composable RowScope.() -> Unit
) {
    OutlinedButton(
 ...
 * @param small Whether or not the size of the button should be small or regular.
 * @param colors [ButtonColors] that will be used to resolve the container and content color for
 * this button in different states. See [TwcButtonDefaults.textButtonColors].
 * @param contentPadding The spacing values to apply internally between the container and the
 * content. See [TwcButtonDefaults.buttonContentPadding].
 * @param content The button content.
 */
...
fun TwcTextButton(
    ...
    enabled: Boolean = true,
    small: Boolean = false,
    colors: ButtonColors = TwcButtonDefaults.textButtonColors(),
    contentPadding: PaddingValues = TwcButtonDefaults.buttonContentPadding(small = small),
    content: @Composable RowScope.() -> Unit
) {
    TextButton(
 ...
 */
@Composable
private fun RowScope.TwcButtonContent(
    text: @Composable () -> Unit,
    leadingIcon: @Composable (() -> Unit)?,
    trailingIcon: @Composable (() -> Unit)?
) {
    if (leadingIcon != null) {
        Box(Modifier.sizeIn(maxHeight = TwcButtonDefaults.ButtonIconSize)) {
            leadingIcon()
        }
    }
    Box(
        Modifier
            ...
            .padding(
                start = if (leadingIcon != null) {
                    TwcButtonDefaults.ButtonContentSpacing
                } else {
                    0.dp
                },
